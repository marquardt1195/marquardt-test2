...
#ATTEMPT TO CALCULATE AVG WIN AND LOSE HOLD TIMES WITHOUT USING HOLD TIME IN EXCEL
#df_entry_date <- trade_results_final %>% 
#    select(trade, stock_name, 
#         entry_date, exit_date) %>% 
#    group_by(trade) %>% 
#    summarise("entry_dates" = tail(entry_date))
#
#df_exit_date <- trade_results_final %>% 
#    select(trade, stock_name, 
#         entry_date, exit_date) %>% 
#    group_by(trade) %>% 
#    summarise("exit_dates" = tail(exit_date))
#
#df_hold_time <- merge(df_entry_date, df_exit_date) %>% 
#  group_by(trade) %>% 
#  drop_na() %>% 
#  summarise("hold_time" = as.Date(as.character(df_hold_time$exit_dates), format = "%m/%d/%Y") -
#           as.Date(as.character(df_hold_time$entry_dates), format = "%m/%d/%Y"))
#
...
df_monthly_data13 <- df_monthly_data1 %>% #Monthly avg hold time of winners
  ...
  group_by(month) %>% 
  filter(pl_percent_trade > 0.5) %>% 
  summarise("avg_win_hold" = mean(pl_percent_trade)) %>% 
  arrange(match(month, month.name))
...
df_monthly_data14 <- df_monthly_data1 %>% #Monthly avg hold time of losers
  ...
  group_by(month) %>% 
  filter(pl_percent_trade < -0.5) %>% 
  summarise("avg_loss_hold" = mean(pl_percent_trade)) %>% 
  arrange(match(month, month.name))
